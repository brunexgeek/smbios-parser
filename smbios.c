...
static void parse_bios_info(struct ParserContext *context)
{
    ...
    if (context->sversion >= SMBIOS_2_0)
    {
        ...
        context->entry.data.bios.Vendor          = get_string(context, context->entry.data.bios.Vendor_);
        context->entry.data.bios.BIOSVersion     = get_string(context, context->entry.data.bios.BIOSVersion_);
        context->entry.data.bios.BIOSReleaseDate = get_string(context, context->entry.data.bios.BIOSReleaseDate_);
    }
    // 2.4+
...
static void parse_system_info(struct ParserContext *context)
{
    ...
    if (context->sversion >= SMBIOS_2_0)
    {
        ...
        context->entry.data.sysinfo.Manufacturer = get_string(context, context->entry.data.sysinfo.Manufacturer_);
        context->entry.data.sysinfo.ProductName  = get_string(context, context->entry.data.sysinfo.ProductName_);
        context->entry.data.sysinfo.Version = get_string(context, context->entry.data.sysinfo.Version_);
        context->entry.data.sysinfo.SerialNumber = get_string(context, context->entry.data.sysinfo.SerialNumber_);
    }
    // 2.1+
    ...
    if (context->sversion >= SMBIOS_2_4)
    {
        ...
        context->entry.data.sysinfo.SKUNumber = get_string(context, context->entry.data.sysinfo.SKUNumber_);
        context->entry.data.sysinfo.Family = get_string(context, context->entry.data.sysinfo.Family_);
    }
}
...
static void parse_baseboard_info(struct ParserContext *context)
{
    ...
    if (context->sversion >= SMBIOS_2_0)
    {
        ...
        context->entry.data.baseboard.Manufacturer      = get_string(context, context->entry.data.baseboard.Manufacturer_);
        context->entry.data.baseboard.Product           = get_string(context, context->entry.data.baseboard.Product_);
        context->entry.data.baseboard.Version           = get_string(context, context->entry.data.baseboard.Version_);
        context->entry.data.baseboard.SerialNumber      = get_string(context, context->entry.data.baseboard.SerialNumber_);
        context->entry.data.baseboard.AssetTag          = get_string(context, context->entry.data.baseboard.AssetTag_);
        context->entry.data.baseboard.LocationInChassis = get_string(context, context->entry.data.baseboard.LocationInChassis_);
    }
}
...
static void parse_system_enclosure(struct ParserContext *context)
{
    ...
    if (context->sversion >= SMBIOS_2_0)
    {
        ...
        context->entry.data.sysenclosure.Manufacturer = get_string(context, context->entry.data.sysenclosure.Manufacturer_);
        context->entry.data.sysenclosure.Version      = get_string(context, context->entry.data.sysenclosure.Version_);
        context->entry.data.sysenclosure.SerialNumber = get_string(context, context->entry.data.sysenclosure.SerialNumber_);
        context->entry.data.sysenclosure.AssetTag     = get_string(context, context->entry.data.sysenclosure.AssetTag_);
    }
    // 2.1+
    if (context->sversion >= SMBIOS_2_1)
    {
    ...
    if (context->sversion >= SMBIOS_2_7)
    {
        ...
        context->entry.data.sysenclosure.SKUNumber = get_string(context, context->entry.data.sysenclosure.SKUNumber_);
    }
}
...
static void parse_processor_info(struct ParserContext *context)
{
    ...
    if (context->sversion >= SMBIOS_2_0)
    {
        ...
        context->entry.data.processor.SocketDesignation     = get_string(context, context->entry.data.processor.SocketDesignation_);
        context->entry.data.processor.ProcessorManufacturer = get_string(context, context->entry.data.processor.ProcessorManufacturer_);
        context->entry.data.processor.ProcessorVersion      = get_string(context, context->entry.data.processor.ProcessorVersion_);
    }
    // 2.1+
    if (context->sversion >= SMBIOS_2_1)
    {
    ...
    if (context->sversion >= SMBIOS_2_3)
    {
        ...
        context->entry.data.processor.SerialNumber = get_string(context, context->entry.data.processor.SerialNumber_);
        context->entry.data.processor.AssetTagNumber = get_string(context, context->entry.data.processor.AssetTagNumber_);
        context->entry.data.processor.PartNumber = get_string(context, context->entry.data.processor.PartNumber_);
    }
    // 2.5+
...
static void parse_system_slot(struct ParserContext *context)
{
    ...
    if (context->sversion >= SMBIOS_2_0)
    {
        ...
        context->entry.data.sysslot.SlotDesignation = get_string(context, context->entry.data.sysslot.SlotDesignation_);
    }
    // 2.1+
    if (context->sversion >= SMBIOS_2_1)
    {
...
static void parse_memory_device(struct ParserContext *context)
{
    ...
    if (context->sversion >= SMBIOS_2_1)
    {
        ...
        context->entry.data.memory.DeviceLocator  = get_string(context, context->entry.data.memory.DeviceLocator_);
        context->entry.data.memory.BankLocator    = get_string(context, context->entry.data.memory.BankLocator_);
    }
    // 2.3+
    if (context->sversion >= SMBIOS_2_3)
    {
        ...
        context->entry.data.memory.Manufacturer   = get_string(context, context->entry.data.memory.Manufacturer_);
        context->entry.data.memory.SerialNumber   = get_string(context, context->entry.data.memory.SerialNumber_);
        context->entry.data.memory.AssetTagNumber = get_string(context, context->entry.data.memory.AssetTagNumber_);
        context->entry.data.memory.PartNumber     = get_string(context, context->entry.data.memory.PartNumber_);
    }
    // 2.6+
    if (context->sversion >= SMBIOS_2_6)
    {
...
static void parse_port_connector(struct ParserContext *context)
{
    ...
    if (context->sversion >= SMBIOS_2_0)
    {
        ...
        context->entry.data.portconn.ExternalReferenceDesignator = get_string(context, context->entry.data.portconn.ExternalReferenceDesignator_);
        context->entry.data.portconn.InternalReferenceDesignator = get_string(context, context->entry.data.portconn.InternalReferenceDesignator_);
    }
}
...
static void parse_management_device(struct ParserContext *context)
{
    if (context->sversion >= SMBIOS_2_0)
    {
        ...
        context->entry.data.mdev.Description = get_string(context, context->entry.data.mdev.Description_);
    }
}
...
static void parse_onboard_devices_extended_info(struct ParserContext *context)
{
    if (context->sversion >= SMBIOS_2_0)
    {
        ...
        context->entry.data.odeinfo.ReferenceDesignation = get_string(context, context->entry.data.odeinfo.ReferenceDesignation_);
    }
}
